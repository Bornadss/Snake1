...
void GameSnake::GameForm::GeneratePositionFruit()
{
	...
	//10 - смещение относительно боков формы
	positionFruit.Y = rand->Next(90, gameArea.Y);
	//90 - смещение верхней граници относительно верха формы
...
void GameSnake::GameForm::Eating()
{
	...
	if (Serpens[0]->Location.X == positionFruit.X && Serpens[0]->Location.Y == positionFruit.Y) {
		...
		Serpens[score]->Location = Point(Serpens[score - 1]->Location.X + step * direction.X,
			Serpens[score - 1]->Location.Y - step * direction.Y);
...
void GameSnake::GameForm::NewGame()
{
	...
	//голова будет зеленой, чтобы отличалась от всех
...
{
	if (Serpens[0]->Location.X >= RigtBorder->Location.X || Serpens[0]->
		Location.X <= LeftBorder->Location.X) {
		//direction.X *= -1;//развернуть в обратном направлении
		GameOver();
	}

	if (Serpens[0]->Location.Y <= UpperBorder->Location.Y || Serpens[0]->
		Location.Y >= LowerBorder->Location.Y) {
		//direction.Y *= -1;//развернуть в обратном направлении
		GameOver();
	}

	//Вывод координат змейки
	labelCoordinatesSnake->Text = "Кооринаты змейки: (" + Convert::ToString(Serpens[0]->
		Location.X) +
...
//------------------
//Реализация событий
System::Void GameSnake::GameForm::выходToolStripMenuItem_Click(System::Object^ sender,
	System::EventArgs^ e)
{
	Application::Exit();
}

System::Void GameSnake::GameForm::информацияОИгреToolStripMenuItem_Click(System::Object^ sender,
	System::EventArgs^ e)
{
...
System::Void GameSnake::GameForm::новаяИграToolStripMenuItem_Click(System::Object^ sender,
	System::EventArgs^ e)
{
	NewGame();
...
System::Void GameSnake::GameForm::паузапродолжитьToolStripMenuItem_Click(System::Object^ sender,
	System::EventArgs^ e)
{
	if (play) {
		play = false;
...
System::Void GameSnake::GameForm::настройкиToolStripMenuItem_Click(System::Object^ sender,
	System::EventArgs^ e)
{
...
System::Void GameSnake::GameForm::buttonApplySpeed_Click(System::Object^ sender,
	System::EventArgs^ e)
{
...
System::Void GameSnake::GameForm::GameForm_KeyDown(System::Object^ sender,
